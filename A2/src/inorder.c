...
//Printing inorder function definition
void printInOrder (struct node * node){
  ...
  //Printing opening parentheses when appropriate
  if ((node->contents[0] == '+') || (node->contents[0] == '-') || (node->contents[0] == '/') || (node->contents[0] == '*')){
    printf("(");
  }

  //Printing the left subtree recursively, then the root, then the right subtree recursively
  printInOrder(node->left);
  printf("%s",node->contents);
  printInOrder(node->right);

  //Printing closing parentheses when appropriate
  if ((node->contents[0] == '+') || (node->contents[0] == '-') || (node->contents[0] == '/') || (node->contents[0] == '*')){
    printf(")");
  }
  ...
  if ((openingCount == 0) && (closingCount == 0)){
   ...
   int operatorIndex = -1;
    ...
    if (operatorIndex == -1) {
      ...
      //Recursive calls to parse the string to the left and right of the operator
